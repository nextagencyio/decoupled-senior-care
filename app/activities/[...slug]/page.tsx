...
import { GET_ACTIVITY_BY_PATH } from '@/lib/queries'
...
interface ActivityByPathData {
  route: {
    entity: DrupalActivity
  } | null
}

async function getActivity(path: string): Promise<DrupalActivity | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<ActivityByPathData>({
      query: GET_ACTIVITY_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/activities/${slug.join('/')}`
  const item = await getActivity(path)

  if (!item) {
    return { title: 'Activity Not Found | Senior Care' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).instructor && (
                    <div>
                      <dt className="text-sm text-gray-500">Instructor</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).instructor}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
