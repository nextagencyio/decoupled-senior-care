...
import { GET_COMMUNITY_BY_PATH } from '@/lib/queries'
import { DrupalCommunity } from '@/lib/types'
...
interface CommunityByPathData {
  route: {
    entity: DrupalCommunity
  } | null
}

async function getCommunity(path: string): Promise<DrupalCommunity | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<CommunityByPathData>({
      query: GET_COMMUNITY_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/communities/${slug.join('/')}`
  const item = await getCommunity(path)

  if (!item) {
    return { title: 'Community Not Found | Senior Care' }
  }
